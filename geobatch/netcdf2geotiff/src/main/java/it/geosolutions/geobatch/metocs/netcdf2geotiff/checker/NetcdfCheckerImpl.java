...
public class NetcdfCheckerImpl<TYPE> extends NetcdfChecker<TYPE> {
    ...
    private void setTime(final Variable var) {
        ...
        if (timeVar == null) {
            if (LOGGER.isWarnEnabled())
                LOGGER.warn("Unable to get the time for this varibale, check your dictionary");
            time = null;
            timeDimExists = false;
        } else {
            try {
    ...
    private boolean setLat(final Variable var) {
        ...
        if (latVar == null) {
            if (LOGGER.isWarnEnabled())
                LOGGER.warn("Unable to get the Latitude for varibale: \'" + var.getName()
        ...
        try {
            ...
            if (LOGGER.isWarnEnabled())
                LOGGER.warn("Unable to read the Latitude array for varibale: \'" + var.getName()
                            + "\', check your dictionary");
            lat = null;
            latDimExists = false;
        }
    ...
    public String buildName(final Variable var, final int... coords) {
        ...
        if (tau != null) {
            synchronized (sdf) {
                coverageName
                    .append(Integer.parseInt(tau) == 0 ? runTime : timeDimExists
                                ? sdf.format(super.getTimeInstant(localBaseTime, time, coords[0],
                                                                  timeConversion)) : "00000000T000000000Z")
