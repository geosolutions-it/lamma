...
public Map execute(Map argsMap) throws Exception {
	...
	final List<FileSystemEvent> mosaicOutEvents = (List)argsMap.get(ScriptingAction.RETURN_KEY)
        final Map<File, Map> rootMap = ImageMosaicUtils.loadMetadataMap(mosaicOutEvents);
...
//        argsMap = MeteosatUtils.GeoNetworkAndGeoStore(argsMap);
        ...
        logger.info("Publishing on geonetwork");
        for(Object o : rootMap.keySet()) {
           logger.info(" - event file " + o);
        }
        GeoNetworkUtils.publishOnGeoNetworkAction(listenerForwarder,failIgnore,tempDir,configDir,rootMap,argsMap, map);
