...
import it.geosolutions.geobatch.lamma.misc.MergeImageUtils;
...
public Map execute(Map argsMap) throws Exception {
	...
	final Logger LOGGER = LoggerFactory.getLogger("it.geosolutions.geobatch.action.scripting.ScriptingAction.class");
		...
		if (LOGGER.isDebugEnabled())
		      LOGGER.debug("::Categorical : event: "+event);
//System.out.println("--------------> event: "+event);

		final File eventFile=event.getSource();
		final String fileName = eventFile.getName();
		if (imcP.matcher(fileName).matches()) {

//System.out.println("--------------> event matches ");
		  if (LOGGER.isDebugEnabled()) {
		      LOGGER.debug("::Categorical : event matches ");
		      LOGGER.debug("::Categorical : working on incoming event: "+fileName);
		  }
		  ...
		  if (outDir==null){
			  ...
			  LOGGER.debug("::Categorical : event matches ");
			  LOGGER.debug("::Categorical : working on incoming event: "+fileName);
			  LOGGER.debug("::Categorical : initializing outDir")
		      }
//System.out.println("--------------> initializing outDir");
			...
			if (LOGGER.isDebugEnabled()) {
			  LOGGER.debug("::Categorical : model file name: "+layerModelFileName);
			  LOGGER.debug("::Categorical : prefixRegex: "+prefixRegex);
			}
//System.out.println("--------------> model file name: "+layerModelFileName);
//System.out.println("--------------> prefixRegex: "+prefixRegex);
//System.out.println("--------------> ERROR: No match found");
			final String message="::Categorical : ERROR: No match found";
			...
			if (LOGGER.isDebugEnabled()) {
			  LOGGER.debug("::Categorical : model file name: "+layerModelFileName);
			  LOGGER.debug("::Categorical : suffixRegex: "+suffixRegex);
			}
//System.out.println("--------------> model file name: "+layerModelFileName);
//System.out.println("--------------> suffixRegex: "+suffixRegex);
			...
			LOGGER.debug("::Categorical : prefixRegex: "+prefixRegex);
			LOGGER.debug("::Categorical : suffixRegex: "+suffixRegex);
			LOGGER.debug("::Categorical : preMatch: "+imcPrefixM.group(1));
			LOGGER.debug("::Categorical : sufMatch: "+imcSuffixM.group(1));
		      }
...
//System.out.println("--------------> sufMatch: "+imcSuffixM.group(1));

		      final String outDirFileName=imcPrefixM.group(1)+outDirName+imcSuffixM.group(1);
		      if (LOGGER.isDebugEnabled()) {
			  LOGGER.debug("::Categorical : layerName: "+outDirFileName);
		      }
//System.out.println("--------------> layerName: "+outDirFileName);
	    ...
	    if (outDir==null){
		final String message="::Categorical : ERROR: unable to get the output directory from the incoming IMC list";
		if (LOGGER.isErrorEnabled())
		      LOGGER.error(message);
		final Exception e=new Exception(message);
		listenerForwarder.failed(e);
...
//System.out.println("ERROR: unable to get the output directory from the incoming IMC list");
		...
		final String message="::Categorical : ERROR: unable to create the output directory: "+outDir.getAbsolutePath();
		if (LOGGER.isErrorEnabled())
		      LOGGER.error(message);
		final Exception e=new Exception(message);
		listenerForwarder.failed(e);
...
//System.out.println("ERROR: unable to create the output directory: "+outDir.getAbsolutePath());
	    ...
	    if (LOGGER.isDebugEnabled()){
		LOGGER.error("::Categorical : Working on a list having SIZE:"+workListSize);
	    }
//System.out.println("SIZE:"+workListSize);
//System.out.println("PATH:"+workList.get(0).getSource().getAbsolutePath());
	    ...
	    for (String fileName in workList){
	    	...
	    	if (cmd==null){
		      // ERROR
		      if (LOGGER.isErrorEnabled())
			LOGGER.error("::Categorical : SKIPPING: NULL ImageMosaicCommand");
		      continue;
	    	}
			...
			final String message="::Categorical : ERROR: here we suppose that each image mosaic command has the same number of file!!!";
			if (LOGGER.isErrorEnabled())
			      LOGGER.error(message);
			final Exception e=new Exception(message);
			listenerForwarder.failed(e);
		...
		int j=0;
		    ...
		    if (imagesList==null){
			    // ERROR
			final String message="::Categorical : ERROR: image list is null";
			if (LOGGER.isErrorEnabled())
			      LOGGER.error(message);
			final Exception e=new Exception(message);
			listenerForwarder.failed(e);
		    ...
		    if (image==null){
			    // ERROR
			final String message="::Categorical : ERROR: selected image is null";
			if (LOGGER.isErrorEnabled())
			      LOGGER.error(message);
			final Exception e=new Exception(message);
			listenerForwarder.failed(e);
		...
		File newCommandFile;
	    	try {
		    	final File mergedImage=util.mergeMaskImage(outDir,args,outDirName);
			addFilesList.add(mergedImage);
	    	} catch (Throwable t){
			final String message="::Categorical : ERROR:"+t.getLocalizedMessage();
			if (LOGGER.isErrorEnabled())
			      LOGGER.error(message);
			final Exception e=new Exception(message);
			listenerForwarder.failed(e);
...
//System.out.println("ERROR: unable to build the IMC...");
		final String message="::Categorical : ERROR: unable to build the IMC...";
		if (LOGGER.isErrorEnabled())
		    LOGGER.error(message);
		final Exception e=new Exception(message);
		listenerForwarder.failed(e);
	    ...
	    final File newCommandFile=newCommand.serialize(newCommand, outDir.getAbsolutePath()+"/Categorical_IMC.xml");
	    ret.add(newCommandFile.getAbsolutePath());
// REMOVE FILES:
	    for (String fileName in workList){
	    	final File file=new File(fileName).getParentFile();
		if (LOGGER.isDebugEnabled())
		    LOGGER.debug("::Categorical : going to remove: "+file.getAbsolutePath());
